...
class ModelSB3(Model.Model):
    ...
    def checkpoint_out(self, ckpt_dir):
        self.save(Path(ckpt_dir, 'model__model.zip'))

    def save(self, file_path):
        self.sb3model.save(file_path)

    def load(file_path):
        return Model.read_sb3_model(file_path)

    def predict(self, observations):
        action, _states = self.sb3model.predict(observations, deterministic=True)
        return action

    # includes q-values with action
    def predict2(self, observations):
        self.sb3model.q_net.set_training_mode(False)
        obs_tensor, vectorized_env = self.sb3model.q_net.obs_to_tensor(observations)
        with torch.no_grad():
            q_values = self.sb3model.q_net(obs_tensor)
        q_values = q_values.cpu().numpy().reshape((-1, *self.sb3model.q_net.action_space.shape))
        # Greedy action
