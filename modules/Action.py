...
def step_through(grid, x, y, z, step_function, magnitude, step_size):
    distance_traveled = 0
    valid = True
    while(distance_traveled < magnitude):
        x2, y2, z2 = step_function(x, y, z, step_size)
        valid = (grid.in_bounds(x2, y2, z2) and not grid.in_object(x2, y2, z2))
        if not valid:
            break
        x, y, z = x2, y2, z2
...
class Move(Action):
    def __init__(self, move_function, magnitude, step_size=1):
...
class Forward(Action):
    def __init__(self, magnitude, step_size=1):
        self.magnitude = magnitude
        self.step_size = step_size
