...
class MaxSteps(Terminator):
    ...
    def __init__(self, steps_multiplier, max_steps_dict={}, max_max_steps=99999):
        self.steps_multiplier = steps_multiplier
        self.max_steps_dict = max_steps_dict
        self.max_max_steps = max_max_steps
        
    def start(self, environment):
        path_steps = environment.get_path_steps()
        if path_steps in self.max_steps_dict:
            self.max_steps = self.max_steps_dict[path_steps]
        else:
            self.max_steps = math.ceil(self.steps_multiplier * path_steps)
        self.max_steps = min(self.max_steps, self.max_max_steps)
