...
def read_curriculum(map_name, astar_version, set_name, n_paths=None, difficulties=None, curriculum_version='curriculum'):
    curriculum_dir = Path(DATA_DIR, 'maps', map_name, 'paths', astar_version, curriculum_version)
    ...
    if astar_version in ['old', 'experimental']:
        ...
        for difficulty in difficulties:
            ...
            max_paths = len(paths_in)
            if n_paths is not None:
                max_paths = min(n_paths, len(paths_in))
            for path in paths_in[:max_paths]:
                target_pos = path[0, :3]
                points = []
