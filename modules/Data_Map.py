        ...
        for y in data_dict[x]:
            ...
            for z in data_dict[x][y]:
                ...
                for d in data_dict[x][y][z]:
                    observation = data_dict[x][y][z][d].copy()
...
class DataMap:
    ...
    # if memory_saver=True, cache_size is number of data_maps to keep saved in RAM as loaded from file
    def __init__(self, map_name, memory_saver=False, cache_size=8):
        ...
        self._last_filepaths = [] # keep track of file path to last read data dicts for memory saver
        self._last_data_dicts = {} # keep track of actual data in cache
    ...
    def view_path(self, points, fig=None, ax=None, interval=40):
        make_and_show = False
        if fig is None:
            make_and_show = True
            fig, ax = plt.subplots()
        ...
        self.plot_map(fig, ax, interval=interval)
        for point in points:
            self.plot_agent(fig, ax, point, start_point=start_point, target_point=target_point, show_fov=False)
        if make_and_show:
            plt.show()
    ...
    def plot_agent(self, fig, ax, point, fov_horizion = 255,
        ...
        if start_point is not None:
            ax.scatter(start_point.x+self.x_shift, start_point.y+self.y_shift, color='blue', 
                       marker='x', s=base_marker_size*marker_scale)
        if target_point is not None:
            ax.scatter(target_point.x+self.x_shift, target_point.y+self.y_shift, color='green', 
                       marker='*', s=2*base_marker_size*marker_scale)
        if path is not None:
            for next_point in path:
                ax.scatter(next_point.x+self.x_shift, next_point.y+self.y_shift, color='blue', 
                           marker='x', s=base_marker_size*marker_scale)
    ...
    def animate(self, sensor_metas, observations, nrows, ncols,
                sensor_names=None, points=None, show_fov=False, interval = 40,
        ...
        if show_map:
            ax = axs['map']
            self.plot_map(fig, ax, show_z=True, interval = interval)
        ...
        def update(t):
            ...
            if show_map:
                ...
                ax = axs['map']
                ax.clear()
                self.plot_map(fig, ax, show_z=False, interval = interval)
                if show_path:
    ...
    def get_observation(self, x, y, z, direction, data_dict):
        if x in data_dict and y in data_dict[x] and z in data_dict[x][y] and direction in data_dict[x][y][z]:
            return data_dict[x][y][z][direction]
        else:
            return None
    ...
    def get_data_point(self, point, sensor_name, use_memory_saver=False):
        ...
        if self.memory_saver or use_memory_saver:
            ...
            # observation = self.get_observation(x, y, z, direction, self._last_data_dict)
            # return observation
            # multi cache system
            if filepath not in self._last_data_dicts:
                self._last_filepaths.append(filepath)
                if len(self._last_filepaths) > self.cache_size:
                    remove_filepath = self._last_filepaths[0]
                    del self._last_data_dicts[remove_filepath]
                    self._last_filepaths.remove(remove_filepath)
                    gc.collect()
                self._last_data_dicts[filepath] = Utils.pickle_read(filepath)
            observation = self.get_observation(x, y, z, direction, self._last_data_dicts[filepath])
            return observation
        ...
        observation = self.get_observation(x, y, z, direction, data_dict)
        if observation is None:
            filepath = self.get_filepath(sensor_name, x, y)
    ...
    def make_animation(self, observations, sensor_names, points=None, ncols=2, sensor_psuedonames={}, 
                             fig_scale=4, text_blocks=None, show_path=False, show_fov=False, interval=40):
        ...
        animation = self.animate(sensor_metas, observations, nrows, ncols, interval=interval,
    ...
    # gets all valid grid points
    def get_grid_points(self, z=4):
        if len(self.grid_points) == 0:
            for x in range(self.x_min, self.x_max):
                for y in range(self.y_min, self.y_max):
                    if self.in_bounds(x, y, z) and not self.in_object(x, y, z):
                        self.grid_points.append(Data_Structure.Point(x, y, z))
        return self.grid_points

    # returns valid x,y coords
    def sample_points(self, sample_size):
        grid_points = self.get_grid_points()
        sampled_points = random.sample(grid_points, sample_size)
...
class DataMapRoof(DataMap):
    # child constructor 
    # assumes that x,y increments are a multiple of resolution
    def __init__(self, map_name, memory_saver=False, cache_size=8, collision_threshold=2, roof_name='default', bounds_name='default', resolution=1):
        super().__init__(map_name, memory_saver, cache_size)
        self.collision_threshold = collision_threshold
        self.roof_name = roof_name
        self.resolution = resolution
        roofs_path = Path(DATA_DIR, 'maps', map_name, 'roofs', f'{roof_name}.p')
        self.set_roofs(roofs_path)
        bounds_path = Path(Utils.get_global('data_directory'), 'maps', map_name, 'bounds', f'{bounds_name}.p')
        self.dynamic_bounds = False
        if os.path.exists(bounds_path):
            print('reading bounds from path', bounds_path)
            self.set_bounds(bounds_path)
        self.grid_points = []

    def get_bounds(self, x, y):
        if self.dynamic_bounds:
            x_min, x_max = self.bounds[y]
            y_min, y_max = self.y_min, self.y_max
            for down in range(y-1, self.y_min-1, -1):
                x_min2, x_max2 = self.bounds[down]
                if x < x_min2 or x >= x_max2:
                    y_min = down
                    break
            for up in range(y+1, self.y_max):
                x_min2, x_max2 = self.bounds[up]
                if x < x_min2 or x >= x_max2:
                    y_max = up
                    break
    ...
    def in_bounds2(self, x, y, z):
        if y not in self.bounds:
            return False
        x_low, x_high = self.bounds[y]
        return x >= x_low and x < x_high

    # reads bounds {y:[x_low, x_high]}
    def set_bounds(self, bounds_path):
        print('setting dynamic bounds')
        self.bounds = Utils.pickle_read(bounds_path)
        self.in_bounds = self.in_bounds2
        self.dynamic_bounds = True
        # pre compute bounds at each x,y coord
        # self.precomputed = {}
        # for y in range(self.y_min, self.y_max):
        #     x_min, x_max = self.bounds[y]
        #     for x in range(x_min, x_max):
        #         self.precomputed[x][y] = 

    def get_gemma3_map(self, z=4):
        roofs_arr = self.roofs_array.copy()
        roofs_arr[np.isnan(roofs_arr)] = 0
        roofs_arr[roofs_arr > 255] = 255
        img_arr = roofs_arr.astype(np.uint8)
        img_arr[img_arr >= z - self.collision_threshold] = 255
        img_arr[img_arr < z + self.collision_threshold] = 0
        img_arr = img_arr.T
        img_arr = np.flipud(img_arr)
        
        # pad with zeros
        new_arr = np.full((896, 896), 255, dtype=np.uint8)
        half = int((1000 - img_arr.shape[1])/2)
        new_arr[:img_arr.shape[0], half:(half+img_arr.shape[1])] = img_arr
